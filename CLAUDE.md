...
- **`profile/`** — gitignored. Identity, EEO answers, `[paths]` to the driving docs, fit profile, QA checklist, the resume generator.
  **The driving docs live inside `profile/` as real files**, not as links or absolute paths pointing outside the repo: `profile/inputs/` (resume_master, personal_statement, standard_answers, qa_checklist, steering, cover_letter_examples), `profile/ai_skills/` (claims_ground_truth.md, the resume and cover-letter generators), and `profile/applications/` as the **render target only** — `render()` writes there because it is the one location every surface can write to, and `job-finder applications archive` then moves finished folders to `[paths].applications_archive_dir`, the durable home. That archive is an absolute path outside the repo, so the move only works where that path is granted: natively on Windows, and from Cowork when the archive folder is connected alongside the repo. `profile/` is gitignored, so none of it reaches git. **`profile/` is the source of truth — edit the docs here.** Copies still sitting in OneDrive and `~/.claude/ai_skills` are stale the moment you change one; editing those instead is the failure mode to watch for. Junctions do not work here: Cowork's device bridge resolves a junction to its real target before applying its folder grant, so a linked path reads as the outside folder and fails. Keep these as real files. Relative `[paths]` resolve against the repo root, never the cwd — `job_apply.load_config()` enforces that so the scheduled task keeps working.
...
- `job-finder applications archive [--dry-run]` — move rendered folders from the render target to `[paths].applications_archive_dir`. Never overwrites an existing folder; a name already archived may be a submitted application, so it is skipped and reported.
